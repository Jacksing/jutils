...
# fltr_str = ' and '.join(list(map(combine_param, fltr_str)))
# fltr_fun = eval('lambda x: %s' % fltr_str)

# fltr_matrix = list(filter(fltr_fun, matrix))
# print('%s record%s found.' % (len(fltr_matrix), len(fltr_matrix) > 1 and '(s)' or ''))

# for c in fltr_matrix[0]:
#     print(type(c))

# file_name = datetime.datetime.now().strftime('%Y-%m-%d %H-%M-%S') + '.csv'
# file_name = os.path.join(os.path.dirname(file_path), file_name)
# f = open(file_name, 'w', encoding='utf-8_sig', newline='')
# cw = csv.writer(f)
# cw.writerows(fltr_matrix)
# f.close()

# print('filter result saved to %s' % file_name)